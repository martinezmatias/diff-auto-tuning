...
def main(onlyTest = False):
	...
	for folderToAnalyze in [NAME_FOLDER_ASTJDT, NAME_FOLDER_ASTSPOON]:
		for algorithm in ["Gumtree", "ChangeDistiller", "XyMatcher"]:
			runComputeFitness("{}/{}/".format(RESULTS_ROW_LOCATION, folderToAnalyze),
...
def runComputeFitness(pathResults ,  suffix ="", key = None):

	at_pythom_cmd =  "python3 -m src.rowDataConsumers.RQ0_DistanceLauncher.py {} {} {}".format(pathResults,  suffix, key)
	...
	try:
		if is_grid5k():
			...
			cmd = "oarsub -l nodes=1,walltime=%s -O %s -E %s \"%s\"" % (
				GRID5K_TIME_OUT,
				"./logs/out_distance_{}.txt".format( key),
				"./logs/error_distance_{}.txt".format(key),
				at_pythom_cmd)
