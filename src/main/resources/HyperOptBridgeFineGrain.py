...
rangeGT_BUM_SMT = [ round(x,3) for x in np.arange(0.1,1.1,0.01)]
rangeGT_BUM_SZT = [ x for x in range(100,2001,10)]
...
def recreateConfigurationKey(params):
	...
	for iParameter in propertiesPerMatcher[algo]:
		if str(iParameter).isnumeric():
			iParameter = "{:.3f}".format(iParameter)
...
algoToRun = None
classToRun = None
if tpeparam.lower() == "TPE_HYPEROPTFG".lower():
